...
public final class Crap4jPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        extension.getBaseline().convention(conventionalBaseline);

        TaskProvider<CrapCheck> check = registerAnalysis(
                project, "crapCheck", CrapCheck.class, extension, conventionalBaseline);
        TaskProvider<CrapReport> report = registerAnalysis(
                project, "crapReport", CrapReport.class, extension, conventionalBaseline);
        ...
        project.getPluginManager().withPlugin("java", ignored -> {
            ...
            extension.getJacocoXml().convention(project.getLayout().getBuildDirectory()
    ...
    private static <T extends AbstractCrapAnalysisTask> TaskProvider<T> registerAnalysis(
            ...
            RegularFile conventionalBaseline) {
        return project.getTasks().register(name, type, task -> {
            common(task, extension);
            ...
            task.getConventionalBaseline().convention(conventionalBaseline);
    ...
    private static <T extends AbstractCrapTask> TaskProvider<T> registerBaseline(
        ...
        return project.getTasks().register(name, type, task -> {
            ...
            if (task instanceof CrapBaseline baseline) {
                baseline.getBaseline().convention(extension.getBaseline());
            } else if (task instanceof CrapBaselineTighten tighten) {
                tighten.getBaseline().convention(extension.getBaseline());
            }
        });
    }

    private static void common(AbstractCrapTask task, Crap4jExtension extension) {
